...
detect_docker_run_dir() {
  local socket_path="$1"
  local run_dir="/var/run"

  if [ -n "${socket_path}" ]; then
    run_dir="$(dirname "${socket_path}")"
  fi
...
detect_containerd_root_dir() {
  local configured="${CONTAINERD_ROOT_DIR:-}"
...
CONTAINERD_NAMESPACE="${CONTAINERD_NAMESPACE:-moby}"
